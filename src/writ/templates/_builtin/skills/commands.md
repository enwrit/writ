# writ command reference

Use when the user is running, installing, configuring, or composing
`writ ...` commands; when working with `.writ/` files, instruction
quality scoring, plan review, documentation health, library sync,
or agent-to-agent communication features. The pointer rule
(`writ-context`) loads this skill on demand so the always-on context
stays small.

writ is an optional CLI extension that finds, installs, quality-checks,
and routes AI instructions across coding tools, projects, and agents.
