...
class TestComposer:
    ...
    def test_compose_with_inheritance(
        ...
        store.save_agent(parent_agent)
        store.save_agent(sample_agent_with_parents)
    ...
    def test_compose_with_handoff(self, initialized_project, sample_agent):
        ...
        source = AgentConfig(name="source-agent", instructions="Source instructions")
        store.save_agent(source)
